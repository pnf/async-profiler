...
local int log_replace(struct log *log)
{
    ...
    strcpy(log->end, ".dict");
    dest = (char*) malloc(strlen(log->path) + 1);
    if (dest == NULL)
        return -2;
...
local void log_log(struct log *log, int op, const char *record)
{
    time_t now;
    FILE *rec;
    ...
    if (op == APPEND_OP || op == COMPRESS_OP) {
        ...
        if (stat(log->path, &st) == 0 && st.st_size) {
            ...
            if ((off_t)len != st.st_size ||
                (data = (unsigned char*) malloc(st.st_size)) == NULL) {
...
gzlog *gzlog_open(const char *path)
{
    size_t n;
    struct log *log;
    ...
    n = strlen(path);
    log->path = (char*) malloc(n + 9);              /* allow for ".repairs" */
    if (log->path == NULL) {
        free(log);
        return NULL;
    }
...
int gzlog_compress(gzlog *logd)
{
    ...
    struct log *log = (struct log*) logd;
    ...
    len = ((size_t)(log->last - log->first) & ~(((size_t)1 << 10) - 1)) +
          log->stored;
    if ((data = (unsigned char*) malloc(len)) == NULL)
        return -2;
...
/* gzlog_write() return values:
   ...
   -2: memory allocation failure
   -3: invalid log pointer argument */
int gzlog_write(gzlog *logd, const void *data, size_t len)
{
    int fd, ret;
    struct log *log = (struct log*) logd;
    ...
    if (log_append(log, (unsigned char*) data, len))
        return -1;
