...
int Profiler::getJavaTraceAsync(void* ucontext, ASGCT_CallFrame* frames, int max_depth) {
    ...
    if ((trace.num_frames == ticks_unknown_Java || trace.num_frames == ticks_not_walkable_Java) && _safe_mode < MAX_RECOVERY) {
        ...
        _stubs_lock.lockShared();
        if (_runtime_stubs.contains((const void*)pc)) {
            stub = _runtime_stubs.find((const void*)pc);
        }
        _stubs_lock.unlockShared();

        if (stub != NULL && _cstack != CSTACK_NO) {
            trace.frames->bci = BCI_NATIVE_FRAME;
            trace.frames->method_id = (jmethodID)stub;
            trace.frames++;
            max_depth--;
        }

        // TODO: insert top Java method
        // TODO: Set DebugNonSafepoints via VMStructs or enable CompiledMethodLoad events
        ...
        if (top_frame.validSP()) {
            ...
            bool is_entry_frame = stub != NULL && strcmp(stub, "call_stub") == 0;
        ...
        if (sp != 0 && pc == 0) {
            ...
            if (m != NULL && m->isStub() && m->frameSize() > 0 && m->frameSize() < 256) {
