...
long Profiler::saveAwaitFrames(AwaitFrameType ft, long *elems, int n) {
    ...
    u32 ret = _call_trace_storage.put(n, frames, 1);
    _locks[lock_index].unlock();
    return ret;
}
...
extern "C"
u64 async_profiler_record_custom(int offset, double value, u64 tid_call_trace_id) {
    return Profiler::instance()->recordCustom(offset, value, NULL, tid_call_trace_id);
}

u64 Profiler::recordCustom(int offset, double value, const char* info, u64 tid_call_trace_id) {
    {
    ...
    if (tid_call_trace_id) {
        recordSampleJfr(tid_call_trace_id, 1, BCI_CUSTOM, &e);
        return tid_call_trace_id;
    } else {
        return recordSample(NULL, 1, BCI_CUSTOM, &e);
    }
}

void Profiler::recordSampleJfr(u64 tid_call_trace_id, u64 counter, jint event_type, Event* event) {
